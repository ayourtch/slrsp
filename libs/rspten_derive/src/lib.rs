...
pub fn rsp_handlers_derive(input: proc_macro::TokenStream) -> proc_macro::TokenStream {
    ...
    let gen: proc_macro2::TokenStream = impl_hello_macro(&mut ast).into();
     ...
     let mx_ident: Vec<syn::Expr> = mx.into_iter().map(|x| syn::parse_str(&format!("id_{}", x)).unwrap()).collect();
     // let m1 = mx;
    ...
    let sf_names: Vec<syn::Expr> = struct_field_names.into_iter().map(|x| syn::parse_str(&format!("{}", x)).unwrap()).collect();
    let sf_types: Vec<syn::Expr> = struct_field_types.into_iter().map(|x| syn::parse_str(&format!("{}", x)).unwrap()).collect();
