...
pub struct Deserializer<'de> {
    ...
    input: &'de HashMap<String, Vec<String>>,
    iter: Iter<'de, String, Vec<String>>,
    kv: Option<(&'de String, &'de Vec<String>)>,
}
...
impl<'de> Deserializer<'de> {
    ...
    pub fn from_map(input: &'de HashMap<String, Vec<String>>) -> Self {
        let iter = input.iter();
        let kv = None;
        Deserializer { input, iter, kv }
    }
}
...
pub fn from_map<'a, T>(s: &'a HashMap<String, Vec<String>>) -> Result<T>
where
    T: Deserialize<'a>,
{
    let mut deserializer = Deserializer::from_map(s);
...
impl<'de, 'a> de::Deserializer<'de> for &'a mut Deserializer<'de> {
    ...
    where
        V: Visitor<'de>,
    {
        // println!("deserialize_map: {:#?}", &self.input);
        return visitor.visit_map(&mut self);
        unimplemented!()
        /*
    ...
    {
        if let Some((k, v)) = self.kv {
            return visitor.visit_borrowed_str(&k);
        } else {
...
fn test_struct() {
    ...
    hm.insert("seq".into(), vec!["a".into(), "b".into()]);
